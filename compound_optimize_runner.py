...
def load_data(db_path):
    ...
    rb = co.execute("SELECT timestamp, close FROM candles WHERE symbol='BTCUSDT' AND timestamp>=? ORDER BY timestamp", (start_ts,)).fetchall()
    re = co.execute("SELECT timestamp, close FROM candles WHERE symbol='ETHUSDT' AND timestamp>=? ORDER BY timestamp", (start_ts,)).fetchall()
    fb = co.execute("SELECT timestamp, fundingRate FROM funding WHERE symbol='BTCUSDT' AND timestamp>=? ORDER BY timestamp", (start_ts,)).fetchall()
    fe = co.execute("SELECT timestamp, fundingRate FROM funding WHERE symbol='ETHUSDT' AND timestamp>=? ORDER BY timestamp", (start_ts,)).fetchall()
    co.close()
    ...
    cl_b = cl_b[:n]; cl_e = cl_e[:n]; ts_b = ts_b[:n]
    
    f_cost_b = np.zeros(n); f_cost_e = np.zeros(n)
    for i in range(n):
        t = ts_b[i]
        if t in fund_b: f_cost_b[i] = fund_b[t]
        if t in fund_e: f_cost_e[i] = fund_e[t]
...
def ema(data, period):
    ...
    k = 2 / (period + 1)
    for i in range(period, len(data)): res[i] = data[i] * k + res[i-1] * (1 - k)
    return res
...
def calc_indicators(cl):
    ...
    for i in range(days): ema200_5m[i*288:min((i+1)*288, n)] = ema200_d[i]
        
    h4_count = n // 48
    h4_cl = np.array([cl[min((i+1)*48-1, n-1)] for i in range(h4_count)])
    sma200_h4 = sma(h4_cl, 200)
    sma200_5m = np.zeros(n)
    for i in range(h4_count): sma200_5m[i*48:min((i+1)*48, n)] = sma200_h4[i]
        
    return ema200_5m, sma200_5m

def sim_window(cl_b, cl_e, fcost_b, fcost_e, 
               ema_b, sma_b, ema_e, sma_e,
               lev_stages=(10, 10, 10), thresholds=(280, 400),
               vault_trigger=9999, vault_pct=0.0, use_airbag=False,
               cap=140., tgt=1000., kill=70., fee=.0018,
               leverage_policy="ladder", pi_cfg=None,
               signal_mask_b=None, signal_mask_e=None):
    
    sw = 7 * 288; lb = 14 * 288
    n = len(cl_b)
    
    c = cap; vault = 0.0
    asset = None; coins = 0; ep = 0
    ...
    for i in range(lb, n):
        if coins != 0:
            ...
            if cost_rate != 0: c -= (coins * cl_now) * cost_rate 
                
            eq_active = c + coins * (cl_now - ep)
            eq_total = eq_active + vault
            
            if not reached_t1 and eq_total >= t1_val: reached_t1 = True
            if not reached_t2 and eq_total >= t2_val: reached_t2 = True
            ...
            if eq_total <= kill or c <= 0: 
                if not reached_t1: ruin_before_t1 = True
        ...
        if i % sw == 0:
            ...
            diff = abs(mb - me)
            if diff < whip_threshold: leader = asset 
            else: leader = "BTC" if mb >= me else "ETH"
                
            if leader is not None:
                valid = True
                if ema_b[i] > 0 and (cl_b[i] < ema_b[i] if leader == "BTC" else cl_e[i] < ema_e[i]): valid = False
                if sma_b[i] > 0 and (cl_b[i] < sma_b[i] if leader == "BTC" else cl_e[i] < sma_e[i]): valid = False
                
                current_eq = eq_total if coins != 0 else c + vault
                if use_airbag and i == lb + sw and current_eq < 100: valid = False
                ...
                if not valid: leader = None
                    
            if leader != asset:
                if coins != 0:
                    cl_now = cl_b[i] if asset == "BTC" else cl_e[i]
                    pnl = coins * (cl_now - ep); cost = abs(coins) * cl_now * fee / 2
                    c += pnl - cost; coins = 0
                ...
                if leader is not None:
                    ...
                    pv = c * lev; coins = pv / price; ep = price; asset = leader
                    c -= pv * fee / 2
                else:
                    asset = None
...
def metrics_over_windows(
    *,
    ...
    nw = n_paths
    p_win_given_crossed_t1 = (
        (win_given_t1_cnt / reached_t1_cnt) if reached_t1_cnt > 0 else 0.0
    )
    p_win_given_crossed_t2 = (
        (win_given_t2_cnt / reached_t2_cnt) if reached_t2_cnt > 0 else 0.0
    )
    return {
...
def main():
    ...
    parser.add_argument(
        ...
        help="SQLite con tablas candles y funding (BTCUSDT/ETHUSDT). Por defecto data/candles.db o ../BOTS TRADING/data/candles.db",
    )
    ...
    parser.add_argument('--derisk', type=float, default=0.0, help="Fracción a congelar al cruzar t2")
    parser.add_argument('--early-airbag', type=str, default='false', help="Airbag semana 1 (true/false)")
    parser.add_argument(
        "--holdout-frac",
        type=float,
        default=0.0,
        help="Fraccion temporal OOS al final de las ventanas (0=desactivado). R8 walk-forward liviano.",
    )
    parser.add_argument(
        "--target-equity",
        type=float,
    ...
    parser.add_argument(
        ...
        default="ladder",
        help="ladder: lev1/lev2/lev3 + t1/t2. pi_ref: PI + L^ff (spec docs/specs/2026-04-27-leverage-pid-controller.md).",
    )
    parser.add_argument(
        "--leverage-pi-config",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="JSON v1 de reglas extra (EMA/ADX/ATR/Boll/volumen). Ver docs/signal_rules_examples.md",
    )
    ...
    if not os.path.isfile(db_path):
        err = {
            "error": "db_not_found",
            "path": db_path,
            "hint": "Coloca candles.db en data/, define COMPOUND_OPT_DB, o usa el dataset en ../BOTS TRADING/data/candles.db (misma carpeta PRUEBAS DE PYTHON).",
        }
    ...
    if hf < 0 or hf >= 0.95:
        err = {"error": "invalid_holdout_frac", "holdout_frac": hf, "hint": "Usar 0 < holdout_frac < 0.95"}
        print(json.dumps(err))
        raise SystemExit(2)
    ...
    if tgt <= cap or kill >= cap:
        err = {"error": "invalid_cap_target_kill", "start_capital": cap, "target_equity": tgt, "kill_equity": kill}
        print(json.dumps(err))
        raise SystemExit(2)
